...
module dagon.graphics.materials.morph;
...
class MorphBackend: GLSLMaterialBackend
{
    string vsText =
    "
        #version 330 core
        ...
        layout (location = 4) in vec3 va_Normal2;
        ...
        void main()
        {
            ...
            vec3 newNormal = normalize((1.0f-morphProgress)*va_Normal+morphProgress*va_Normal2);
            eyeNormal = (normalMatrix * vec4(newNormal, 0.0)).xyz;
            vec3 newVertex = (1.0f-morphProgress)*va_Vertex+morphProgress*va_Vertex2;
            vec4 pos = modelViewMatrix * vec4(newVertex, 1.0);
            eyePosition = pos.xyz;

            vec4 position = projectionMatrix * pos;

            blurPosition = blurModelViewProjMatrix * vec4(newVertex, 1.0);
            prevPosition = prevModelViewProjMatrix * vec4(newVertex, 1.0);
